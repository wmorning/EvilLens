...
c
      ...
      double precision defl1(3), defl2(3), defl3(3), defl4(3)
      double precision magmx_xx(3),magmx_xy(3),magmx_yy(3)
      double precision x1(3), x2(3), phi(3)


      do i=1,100
 10      write(*,*) 'Enter axis ratio (<=1; <0 means: end):'
         read(*,*) arat
         if (arat .lt. 0.d0) goto 30
         if (arat .gt. 1.d0) then
            write(*,*) 'Illegal ratio, try again'
            goto 10
         endif
 12      write(*,*) 'Enter q,gam,s2'
         read(*,*) q,gam,s2
         if (s2 .lt. 0.d0) then
            write(*,*) 'Illegal (core rad.)^2, try again'
            goto 12
         endif
         write(*,*) 'Now, evaluation loop'
         do j=1,1
             x1(1) = 1
             x1(2) = 2
             x1(3) = 3
             ...
             x2(2) = 2
             x2(3) = 1
            call fastelldefl_array(x1,x2,q,gam,arat,s2,defl1,defl2, 3)
            write(*,*) 'fastell Defl.= ',defl1(1), defl2(1)
c            call fastellmag_array(x1,x2,q,gam,arat,s2,defl3, defl4,
c           *     magmx_xx,magmx_xy,magmx_yy, 3)
c            write(*,*) 'fastell mag routine: Defl.= ',defl(1,:)
c            write(*,*) 'and Magmx(11,22,12)= ',magmx(1,1,1)
c            write(*,*) magmx(1,2,2),magmx(1,1,2)
            call ellipphi_array(x1,x2,q,gam,arat,s2,phi, 3)
            write(*,*) 'slow Phi first = ',phi(1)
            write(*,*) 'slow Phi second = ',phi(2)
            write(*,*) 'slow Phi third = ',phi(3)
         enddo
 20   enddo
 30   end
c
...
c
      ...
      subroutine ellipphi_array(x1in_array,x2in_array,q,gam,
     *     arat,s2,phi,n)
c     This routine loops over an array of positions and
c     evaluates the subroutine fastellmag, returns arrays
c
c     looping over array elements
      integer n
      double precision x1in_array(n), x2in_array(n), x1in,
     *     x2in, q, gam, arat, s2
      double precision phi(n)
      double precision phi_
      do i=1,n
        phi_ = 0
        x1in = x1in_array(i)
        x2in = x2in_array(i)
        call ellipphi(x1in,x2in,q,gam,arat,s2, phi_)
        phi(i) = phi_
      enddo
      end
...
c
     ...
     *     2.d-7,ans2)
      ...
      subroutine fastellmag_array(x1in_array,x2in_array,q,gam,arat,
     *     s2,defl1,defl2,magmx_xx, magmx_xy, magmx_yy,n)
c     This routine loops over an array of positions and evaluates
c     the subroutine fastellmag, returns arrays
c
c     looping over array elements
      integer n
      double precision x1in_array(n), x2in_array(n)
      double precision q, gam, arat, s2, x1in, x2in
      double precision defl1(n), defl2(n)
      double precision magmx_xx(n), magmx_xy(n), magmx_yy(n)
      double precision defl_(2)
      double precision magmx_(2, 2)
      double precision x(4)
      do i=1,n
        defl_ = ( /0,0 /)
        x = (/0,0,0,0/)
        magmx_ = RESHAPE(x, (/2, 2/))
        x1in = x1in_array(i)
        x2in = x2in_array(i)
        call fastellmag(x1in,x2in,q,gam,arat,s2, defl_, magmx_)
        defl1(i) = defl_(1)
        defl2(i) = defl_(2)
        magmx_xx(i) = magmx_(1,1)
        magmx_xy(i) = magmx_(1,2)
        magmx_yy(i) = magmx_(2,2)
      enddo
      return
      end
...
c     (arat), we make a slight change:      
     ...
     *        fpe2*d1e2-d1s*mres(2)+e1g1*fpe1/x1)
      ...
      subroutine fastelldefl_array(x1in_array,x2in_array,q,
     *     gam,arat,s2,defl1, defl2,n)
c     This routine loops over an array of positions and
c     evaluates the subroutine fastelldefl, returns arrays
c
c     looping over array elements
c
      integer n
      double precision x1in_array(n), x2in_array(n), x1in
      double precision x2in, q, gam, arat, s2
      double precision defl1(n)
      double precision defl2(n)
      double precision defl_(2)
      do i=1,n
        defl_ = (/0, 0/)
        x1in = x1in_array(i)
        x2in = x2in_array(i)
        call fastelldefl(x1in,x2in,q,gam,arat,s2,defl_)
        defl1(i) = defl_(1)
        defl2(i) = defl_(2)
      enddo
      return
